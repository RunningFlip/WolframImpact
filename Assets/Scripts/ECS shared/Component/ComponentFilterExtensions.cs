...
public static class ComponentFilterExtensions
{
    ...
    /// <returns></returns>
    public static List<Entity> ForEach(this List<Entity> _entities, params Type[] _componentTypes)
    {
    ...
    /// <param name="_componentTypes"></param>
    /// <returns></returns>
    public static List<Entity> Without(this List<Entity> _entities, params Type[] _componentTypes)
    {
        if (_entities.Count != 0)
        {
            _entities.RemoveAll(e => ComponentManager.HasComponents(e.GetEntityID(), _componentTypes));
